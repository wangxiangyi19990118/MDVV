...
labels_nums = 2  # 类别个数
batch_size = 16  #
resize_height = 256 # 指定存储图片高度
resize_width = 256  # 指定存储图片宽度
depths = 3
weight_decay=0.1
...
def net_evaluation(sess,loss,accuracy,val_images_batch,val_labels_batch,val_nums):
    ...
    for _ in range(val_max_steps):
        ...
        if(val_y[0][0]==1 and val_acc==1.0):
            TP=TP+1
        '''       if(val_y[0][1]==1 and val_acc==1.0):
            TP=TP+1
        if(val_y[0][2]==1 and val_acc==1.0):
            TP=TP+1
        if(val_y[0][3]==1 and val_acc==1.0):
            TP=TP+1
        if(val_y[0][4]==1 and val_acc==1.0):
            TP=TP+1
            '''
        if (val_y[0][0] == 0 and val_acc == 0.0):
            FP=FP+1
            '''
        if (val_y[0][1] == 0 and val_acc == 0.0):
            FP=FP+1
        if (val_y[0][2] == 0 and val_acc == 0.0):
            FP=FP+1
        if (val_y[0][3] == 0 and val_acc == 0.0):
            FP=FP+1
        if (val_y[0][4] == 0 and val_acc == 0.0):
            FP=FP+1
            '''

        if (val_y[0][0] == 1 and val_acc == 0.0):
            FN=FN+1
            '''
        if (val_y[0][1] == 1 and val_acc == 0.0):
            FN=FN+1
        if (val_y[0][2] == 1 and val_acc == 0.0):
            FN=FN+1
        if (val_y[0][3] == 1 and val_acc == 0.0):
            FN=FN+1
        if (val_y[0][4] == 1 and val_acc == 0.0):
            FN=FN+1
            '''
    ...
    precision=0
    if(TP+FP>0):
        mean_recall=TP/(TP+FP)
    if(TP+FN>0):
        precision=TP/(TP+FN)
    #print(mean_recall)
    return mean_loss, mean_acc,mean_recall,precision
...
def step_train(train_op,loss,accuracy,
    ...
    with tf.Session() as sess:
       # saver = tf.train.import_meta_graph('C:/Users/73136/Documents/Git/M/venv/Include/models/modeld.ckpt-2.meta')
       # saver.restore(sess, tf.train.latest_checkpoint('C:/Users/73136/Documents/Git/M/venv/Include/models/'))
        ...
        for i in range(max_steps + 1):
           ...
           # if i % train_log_step == 0:
            ...
            #if i % val_log_step == 0:
            #print(val_nums)
            mean_loss, mean_acc,mean_recall,precision = net_evaluation(sess, loss, accuracy, val_images_batch, val_labels_batch, val_nums)
            print("%s: Step [%d]  val Loss : %f, val accuracy :  %g，val  precision :  %g,val recall:  %g" % (datetime.now(), i, mean_loss, mean_acc,mean_recall,precision))
...
def train(train_record_file,
    ...
    val_images_batch, val_labels_batch = get_batch_images(val_images, val_labels,
                                                          batch_size=1, labels_nums=labels_nums,
...
if __name__ == '__main__':
    sys.stdout = Logger("1013.log", sys.stdout)
    sys.stderr = Logger("1013.log_file", sys.stderr)  # redirect std err, if necessary

    train_record_file='../dataset/record/train256.tfrecords'
    val_record_file='../dataset/record/val256.tfrecords'

    train_log_step=100
    #weights='xavier'
    base_lr = 0.01  # 学习率
    max_steps =10000   # 迭代次数HarrlikepictureSimhash20
    train_param=[base_lr,max_steps]

    val_log_step=200
    snapshot=500#保存文件间隔
    snapshot_prefix='../models/modeld.ckpt'
